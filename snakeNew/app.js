...
class Game {
    ...
    doTick() {
        ...
        if (this.board.isHeadOnFood()) {
            ...
            this.alertScore(); //выводит значение счета
        }
    ...
    isGameWon() {
        if (this.snake.body.length == this.settings.winLength) {
            clearInterval(this.tickIdentifier);
            this.setMessage("Вы выиграли! Счет " + (this.snake.body.length - 1));
            return true;
        }
        return false;
    }
   ...
   /**
    ...
    //Метод выводит значение счета 
       alertScore() {
        this.score = this.snake.body.length;
        this.setMessage("Счет " + (this.score -1));  
...
class Snake {
    ...
    changeDirection(newDirection) {
        ...
        if (this.isPassedOppositeDirection(newDirection)) {
             clearInterval(this.game.tickIdentifier);                 //добавил остановку игры после проверки движения
            this.game.setMessage("Вы проиграли! Счет " + this.body.length); 
            return ;
        }
